...
do jlong=1,klong
  !
  !-------------------------------------------------
  ! Le test ci-dessous (valeur absolue < à 1.e-30) a été introduit 
  ! le 2010-12-27 par J.M. Piriou pour ne pas
  ! que le mode "trapping" du gfortran ne déclenche
  ! une erreur lors de la conversion vers R4 de réels R8
  ! petits en valeurs absolue. Il s'agit d'un problème gfortran et non LFA.
  ! Ce test pourra donc être supprimé
  ! lorsqu'une version ultérieure de gfortran aura corrigé cette erreur gfortran.
  !-------------------------------------------------
  !
  if(abs(preel(jlong)) < 1.e-30) preel(jlong)=0.
...
&       ,'avant des donnees!...'
  ...
  if(iprec == jpreeusr) then
    ...
    ! Lecture directe donc.
    !
    read(unit=kul) (preel(jlong),jlong=1,klong)
  elseif(iprec == 4) then
    !
...
implicit none
#include"lfatail.h"
integer(kind=jpintusr) iuls,jarg,iltype,iule,iarg,iargcp &
&   ,inomal,ilvalfic,ientier(1),ientierloc,igol1,iprec,inarg
real(kind=jpreeusr) zreel(1),zreelloc
logical llex
character*200 clfs,clnoma,clvalfic,cltype
character*3 cllang
!
! Saisie de la ligne de commande.
!
...
iarg=iargcp()
if(modulo(iarg-1,3) /= 0) then
  !
  !-------------------------------------------------
  ! Le nombre d'arguments n'est pas du type (3n+1).
  ! C'est un problème. Affichage de la documentation.
  !-------------------------------------------------
  !
  if(cllang() == 'FRA') then
    write(*,fmt='(9a)') ' '
...
integer(kind=jpintusr) iarg4,iargcp
character*200 clf
character*3 cllang
!
! Saisie de la ligne de commande.
...
parameter(jplis=1800) ! nombre maxi d'articles dans les fichiers LFA d'entree.
...
integer(kind=jpintusr) iversion
integer(kind=jpintesb) iversionesb
logical ldlfa,llex,lldebug
#include"lfayom.h"
character*(*) cdnomf
character*200 classign
!
!-------------------------------------------------
! Mode debugging du logiciel.
!-------------------------------------------------
!
lldebug=.true.
lldebug=.false.
...
!
inquire(file=cdnomf,exist=llex)
if(lldebug) write(*,fmt=*) 'lfatest/debug :  cdnomf,llex=',trim(cdnomf),llex
if(.not.llex) then
  !
  ! Le fichier n'existe pas.
  ...
  if(lldebug) write(*,fmt=*) 'lfatest/debug :  jpintesb=',jpintesb
  if(lldebug) write(*,fmt=*) 'lfatest/debug :  iversionesb=',iversionesb
  iversion=iversionesb
  close(kul)
#ifdef cray
  call lfaiunassign(kul)
  ...
  if(iversion == 8.or.iversion == 9.or.iversion == 21) then
    ...
    ! attendue.
    !
    if(lldebug) write(*,fmt=*) 'lfatest/debug :  version OK: =',iversion
    ldlfa=.true.
  else
    !
    ...
    ! attendue.
    !
    if(lldebug) write(*,fmt=*) 'lfatest/debug :  version non attendue: =',iversion
    ldlfa=.false.
  endif
  return
  100   continue
  ...
  !
  if(lldebug) write(*,fmt=*) 'lfatest/debug :  pb fichier LFA, sortie sur erreur.'
  close(kul)
#ifdef cray
  call lfaiunassign(kul)
#endif
...
implicit none
#include"lfatail.h"
integer(kind=jpintusr) inile,iarg,iargcp,jarg
...
implicit none
#include"lfatail.h"
real(kind=jpreeusr) prepar(krepar)
real(kind=jpreeusr) pnile(knile-1)
real(kind=jpreeusr) pbord(0:krepar)
integer(kind=jpintusr) jnile,knile,jrepar,krepar
logical :: llok
real(kind=jpreeusr) zproba
do jnile=1,knile-1
...
subroutine lfapplfacop
! --------------------------------------------------------------------------
! **** *LFAPPLFACOP* Copie de n articles d'un fichier LFA sur un autre.
! **** *LFAPPLFACOP* Copy n articles from a LFA file to another.
! --------------------------------------------------------------------------
! Sujet:
! ------
! Arguments explicites:
...
! Externes:
! ---------
! Auteur:   2004-11, J.M. Piriou.
! -------
! Modifications:
! --------------------------------------------------------------------------
implicit none
#include"lfatail.h"
character*200 clnomf,clnomfs,clna
integer(kind=jpintusr) iul,iuls,ilong,ierr,ilnomf,inarg
character*2 cltype
character*3 cllang
integer(kind=jpintusr) iargcp,iarg,jarg
!
! Saisie de la ligne de commande.
!
iarg=iargcp()
if(iarg < 3) then
  if(cllang() == 'FRA') then
    write(*,*) ' '
    write(*,*) 'Copie de n articles d''un fichier LFA sur un autre fichier LFA.'
    write(*,*) ' '
    write(*,*) 'Utilisation: lfacop LFA1 LFA2 ART1 [ART2 ... ARTn]'
    write(*,*) 'avec'
    write(*,*) '    LFA1 nom du fichier LFA d''entree.'
    write(*,*) '    LFA2 nom du fichier LFA de sortie.'
    write(*,*) '    ART1 [ART2 ... ARTn] liste des articles a copier.'
    write(*,*) ' '
    stop
  else
    write(*,*) ' '
    ...
    write(*,*) '    ART1 [ART2 ... ARTn]: articles list.'
    write(*,*) ' '
    stop
  endif
endif
...
!
inarg=2
call getargp(inarg,clnomfs)
iuls=42
call lfaouv(iuls,clnomfs,'W')
!
!-------------------------------------------------
! Fichier d'entrée.
!-------------------------------------------------
...
!
do jarg=3,iarg
  call getargp(jarg,clna)
  call lfacop(iul,clna,clna,iuls)
enddo
end
